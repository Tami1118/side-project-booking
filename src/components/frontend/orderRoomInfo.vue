    ...
    <div class="flex flex-col gap-6 lg:gap-10">
      ...
      <div class="flex flex-col gap-6 text-3h lg:text-4 text-neutral-80 font-bold pb-6 border-b border-neutral-40">
        <div class="flex gap-4 lg:text-5">
          <p class="pe-4 border-r border-neutral-60">{{ order.roomId?.name }}，1晚</p>
          <p>住宿人數：{{ order.peopleNum }} 位</p>
        </div>
        <div>
          <div class="title-deco ps-4 mb-2">入住：{{ $formats.getLocalDateFormat(order.checkInDate) }}，15:00 可入住</div>
          <div class="title-deco ps-4">退房：{{ $formats.getLocalDateFormat(order.checkOutDate) }}，12:00 前退房</div>
        </div>
        <div>NT$ {{ order.roomId?.price }}</div>
      </div>
      <div>
        ...
        <div class="rounded-2 border border-neutral-40 p-6">
          <ul class="flex flex-wrap gap-x-10 gap-y-2">
            <template v-for="(item, key) in order.roomId?.amenityInfo" :key="key">
      ...
      <div>
        ...
        <div class="rounded-2 border border-neutral-40 p-6">
          <ul class="flex flex-wrap gap-x-10 gap-y-2">
            <template v-for="(item, key) in order.roomId?.facilityInfo" :key="key">
              <li class="flex items-center gap-2 text-neutral-80 font-bold" v-if="item.isProvide">
                <span class="material-icons text-primary-100">done</span>
                {{ item.title }}
              </li>
