  ...
  <div class="h-[206px] lg:h-[384px] relative">
    ...
    <div class="absolute top-0 left-0 w-full h-full my-auto">
      <div class="container mx-auto h-full flex flex-col justify-center lg:flex-row lg:justify-start lg:items-center gap-4 lg:gap-6 px-4 sm:px-0 duration-300">
        <div class="w-[72px] h-[72px] lg:w-[144px] lg:h-[144px] rounded-full">
          <img src="/svg/userImage.svg">
        </div>
        <h1 class="text-white text-8 lg:text-12">Hello，{{ userInfo.name }}</h1>
      </div>
    </div>
  </div>
  <div class="container mx-auto px-4 sm:px-0 pt-10 lg:pt-20">
    <div class="mb-10">
      <router-link to="/user" class="py-4 px-6" :class="route.name === 'user'? 'text-primary-100':'text-white'">個人資料</router-link>
      <router-link to="/user-order" class="py-4 px-6"  :class="route.name === 'user-order'? 'text-primary-100':'text-white'">我的訂單</router-link>
    </div>
  </div>
</template>